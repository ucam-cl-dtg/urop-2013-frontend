...
(function($){
	...
	$.jNotify = {
		defaults: {
			...
			Spacing : 5,					// spacing between notifications
		...
		init:function(msg, options, id) {
			...
			$Div = $.jNotify._construct(id, msg, 'body');
			//alert(offset);
			...
			posTop = $.jNotify.vPos(opts.VerticalPosition) + (offset);
			posLeft = $.jNotify.hPos(opts.HorizontalPosition);

			offset = offset + $Div.outerHeight(false) + opts.Spacing;
		...
		/** Construct DOM **/
		/*******************/
		_construct:function(id, msg, append) {
			$Div = 
			$('<div class="jNotifyBox '+id+'"/>')
